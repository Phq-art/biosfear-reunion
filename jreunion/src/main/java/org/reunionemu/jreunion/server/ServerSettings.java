...
public class ServerSettings {
	...
	private float criticalMultiplier; //% amount of critical damage increase.
	...
	private float mobMutantModifier; //modifier to apply on mob stats.
	
	private long mobRadiusArea; //mob area radius.
	
	private int mobMovement;	//Enable/Disable movements from mobs.

	private int thresholdExpDecreasingPlayerMobLvl; //lvl difference between mob and player, after that difference exp starts dropping
	...
	public void loadFromReference() {
		...
		if (server == null) {
			...
			if (server.checkMembers(new String[] { "MobsMovement" })) {
				// use member from file
				setMobMovement(Integer.parseInt(server.getMemberValue("MobsMovement")));
			} else {
				// use default
				setMobMovement(1);
			}
			...
			if (server.checkMembers(new String[] { "ExpPlayerMobDifference" })) {
				// use member from file
				setThresholdExpDecreasingPlayerMobLvl(Integer.parseInt(server.getMemberValue("ExpPlayerMobDifference")));
			} else {
				// use default
				setThresholdExpDecreasingPlayerMobLvl(5);
			}
	...
	public int getThresholdExpDecreasingPlayerMobLvl() {
		return thresholdExpDecreasingPlayerMobLvl;
	}

	public void setThresholdExpDecreasingPlayerMobLvl(int thresholdExpDecreasingPlayerMobLvl) {
		this.thresholdExpDecreasingPlayerMobLvl = thresholdExpDecreasingPlayerMobLvl;
	}
	...
	public int getMobMovement() {
		return mobMovement;
	}

	public void setMobMovement(int mobMovement) {
